...
| Raft (71) | 67 | ~~Debris Scanner~~ → **Loot Luck Boost** (renombrado: Debris Scanner no correspondía a ningún sistema real de Raft) | modifier | MENTAL_BASE |
...
- `IEffectInterpreter` no define aún un mecanismo de rollback si `Apply()` falla después de un `redeem` exitoso (puntos ya debitados, efecto no aplicado). Decisión pendiente: ¿reintento local, o endpoint de
